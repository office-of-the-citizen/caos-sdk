...
export type ParticipationType =
  | 'QUESTION' | 'REQUEST' | 'CONTRIBUTION' | 'CORRECTION'
  | 'CHALLENGE' | 'APPEAL' | 'REPORT' | 'WATCH' | 'SUBSCRIPTION';

export type ParticipationLifecycleState =
  | 'COMPOSED' | 'IDENTIFIED' | 'CONSENTED' | 'FILED' | 'DISPATCHED'
  | 'DELIVERED' | 'ACKNOWLEDGED' | 'TRANSFERRED' | 'CLARIFIED'
  | 'RESPONDED' | 'SILENCE_RECORDED' | 'ESCALATED' | 'CLOSED' | 'WITHDRAWN';

export type ParticipationOutcome =
  | 'RESPONDED_FULL' | 'RESPONDED_PARTIAL' | 'REFUSED' | 'DEEMED_REFUSED'
  | 'WITHDRAWN' | 'ESCALATED' | 'RESOLVED' | 'TRANSFERRED';
...
// ── Participation Ops types (Control Room dashboard) ──────────────────────
...
export type DecisionOriginEngine =
  | '01_SECURITY' | '02_LEDGER' | '05_PARTICIPATION'
  | '06_UTE' | '07_SOURCES' | '08_EVIDENCE'
  | '09_VERIFICATION' | '10_PROJECTION' | '12_GATEWAY'
  | '13_GRAPH' | 'AI_RUNTIME' | 'OPS';

export type DecisionLevel =
  | 'INFORMATION' | 'WARNING' | 'ADMISSION_HOLD'
  | 'QUARANTINE' | 'SYSTEM_FAILURE' | 'PROVIDER_FAILURE'
  | 'CONSTITUTIONAL_CONFLICT';

export type DecisionOutcome =
  | 'SUCCESS' | 'ADMITTED' | 'DUPLICATE' | 'REJECTED'
  | 'QUARANTINED' | 'SUPERSEDED' | 'FAILED' | 'BLOCKED';
